...
import { default as plugins } from '@graze'
...
export const Step = step => props => {
  ...
  const { useGA } = plugins
  const { ReactGA } = (useGA && useGA()) || {}

  const trackEvent = input => ReactGA ? ReactGA.event(input) : false
    ...
    if (refetch) {
      ...
      res.then(r => {
        ...
        if (step === 1) {
          ...
          const { parseTypes, schemaIssues } = require('../../../graze-graphcms/site/schema')
